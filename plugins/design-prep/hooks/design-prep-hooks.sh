#!/bin/bash
# Auto-approve safe read-only operations for design-prep plugin
# Notion writes (notion-create-pages, notion-update-page) deliberately fall through
# to the normal permission flow — per team rule, writes to Notion must be confirmed.
set -euo pipefail
...
# Auto-approve Notion read-only operations (fetch and search only — writes confirm manually)
case "$tool_name" in
